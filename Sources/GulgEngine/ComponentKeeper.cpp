...
void ComponentKeeper::addEntity(const Entity newEntity) {
	...
	if(!entityExist(newEntity)) { m_components.insert(std::make_pair(newEntity, std::map<std::string, std::shared_ptr<Component>>{})); }
}
...
void ComponentKeeper::addComponent(const Entity currentEntity, const std::string componentName, const std::shared_ptr<Component> newComponent) {
	...
	if(!entityExist(currentEntity)) { m_components.insert(std::make_pair(currentEntity, std::map<std::string, std::shared_ptr<Component>>{})); }
...
void ComponentKeeper::cloneEntity(const Entity entityToClone, const Entity clone) {
	...
	if(entityExist(entityToClone)) {
		...
		for(std::pair<std::string, std::shared_ptr<Component>> currentComponent: m_components[entityToClone]) {
...
std::shared_ptr<Component> ComponentKeeper::getComponent(const Entity currentEntity, const std::string componentName) const {
