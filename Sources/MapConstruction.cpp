...
Gg::Entity getNewEntity(Gg::EntityCreator &creator, 
	...
	Gg::Entity newEntity{creator.createEntity()};
	entitySignatureKeeper.addEntity(newEntity);
	componentKeeper.addEntity(newEntity);
	return newEntity;
}
...
void addComponentToEntity(const Gg::Entity entity, const std::string name, std::shared_ptr<Gg::Component> component,
	...
	componentKeeper.addComponent(entity, name, component);
	signatureKeeper.addToSignature(entity, loader.getSignature(name));
}
...
Gg::Entity constructHexaTile(const unsigned int hexaPixelSize,
	...
	Gg::Entity newEntity{getNewEntity(creator, entitySignatureKeeper, componentKeeper)};
	...
	addComponentToEntity(newEntity, std::string{"TileCenterVertex"}, std::static_pointer_cast<Gg::Component>(centerArray),
							 componentKeeper, entitySignatureKeeper, loader);
	...
	addComponentToEntity(newEntity, std::string{"TileBorderVertex"}, std::static_pointer_cast<Gg::Component>(borderArray),
						componentKeeper, entitySignatureKeeper, loader);
